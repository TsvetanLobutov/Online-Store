...
function doShowAll() {
    ...
    if (CheckBrowser()) {
        var key = "";
        var list = "<tr><th>Item</th><th>Title</th><th>Quantity</th><th>Total Price</th><th>Tota</th></tr>\n";
       // var price = "";
        var i = 0;
...
//if no item exists in the cart
        if (list == "<tr><th>Item</th><th>Quantity</th><th>Price</th><th>Quantity</th><th>Tota</th></tr>\n") {
            list += "<tr><td><i>empty</i></td>\n<td><i>empty</i></td><th>empty</th><th>empty</th></tr>\n";
        }
...
function calculateTotalPrice() {
    ...
    var totalPrice = 0;
	 ...
	 for (i = 0; i <= localStorage.length - 1; i++) {
         key = localStorage.key(i);
         totalPrice +=  JSON.parse(localStorage.getItem(key)).price * JSON.parse(localStorage.getItem(key)).quantity
